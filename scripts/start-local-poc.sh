...
requested_runtime_provider=container
if [[ "${RUNTIME_PROVIDER:-}" == "offline-demo" ]]; then
  requested_runtime_provider=offline-demo
fi
...
if [[ "$requested_runtime_provider" != "offline-demo" ]] && \
  [[ -z "${ARK_API_KEY:-}" || -z "${ARK_MODEL:-}" ]]; then
  log "ARK_API_KEY and ARK_MODEL are required."
  log "Example: ARK_API_KEY=key ARK_MODEL=ep-id ./scripts/start-local-poc.sh"
  log "For the credential-free judge demo, run: RUNTIME_PROVIDER=offline-demo ./scripts/start-local-poc.sh"
  exit 2
fi
...
if [[ "$requested_runtime_provider" != "offline-demo" ]]; then
  ...
  log "Using the offline-demo Runtime provider; Ark and Runtime containers are not required."
fi
...
export RUNTIME_PROVIDER="$requested_runtime_provider"
if [[ "$requested_runtime_provider" != "offline-demo" ]]; then
